...
def generateResourceDisplay(ctrl: Controller, showAll=True) -> Panel:
    ...
    for res in ctrl.res_list:
        resTable.add_row(
            ...
            f"{[u.name for u in res.queue] if showAll else ctrl.prettyPrint(res.queue)}",
...
def liveLoop(ctrl: Controller, sleep_time):
